...
#'
#' Small function that calculates a value to be used to calculate the EQR.
#' This value is derived from the IBI and gives a score between 0 and 1
#' (and could be seen as a standardisation of the IBI).
#' This calculation differs between the old (no sampling method specified)
#' and new calculation method (metrics only calculated for specific sampling
#' methods).
#' For the old method (where IBI is already corrected for the number of
#' metrics),
...
#' maximal scores of the metrics and $x = n / c$ with `n` the number of
