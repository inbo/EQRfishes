...
- `shannon_wiener_index`: calculation of the Shannon Wiener index using all species <!-- later berekening nog toevoegen, of referentie hiernaar -->
...
(For brabeel: if less than 30 individuals, the number of individuals is considered too low to calculate a reliable index.)

After that, a standardised IBI is calculated.
For the older indexes, this is $IBI / 5$.
For the newer indexes, this is
 $(IBI - n_{metrics} / c) / (IBI_{max} - n_{metrics} / c)$
...
$(EQR_{class} - 1) / nclass + (IBI_{std} - ibi_{classmin}) / (nclass * (IBI_{classmax} - IBI_{classmin}))$
