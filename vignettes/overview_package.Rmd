...
information <- tribble(
  ...
  "calculate_metric_measures()", 1, -12, 0, 0.5, "calculate_metric_measures.csv",
...
This datasheet gives all needed information to determine the zonation of a specific location.
...
suppressMessages(
  ...
  select(-"tidal", -"indextypecode", -"zonation_old") %>%
  #mutate()
  pivot_wider(names_from = "width", values_from = "zonation") %>%
  arrange(.data$slope) %>%
  kable()
```
...
suppressMessages(
  ...
  filter(.data$indextypecode == "ZTWA") %>%
  select(-"tidal", -"indextypecode", -"zonation") %>%
  #mutate()
  pivot_wider(names_from = "width", values_from = "zonation_old") %>%
  arrange(.data$slope) %>%
  kable()
```
...
Other columns only give presence/absence.  We here list for each column the species that are present (xxx = 1).
...
columns_relevant <- taxonlist %>%
  select(
    -"taxonname", -"taxoncode", -starts_with("WF_"),
    -starts_with("Recr_"), -starts_with("Gkla_"), -ends_with("_Opm")
  ) %>%
  colnames() %>%
  sort()
for (column in columns_relevant) {
  out <- c(out, knit_expand("_taxontable.Rmd"))
}
```
