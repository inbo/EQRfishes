...
suppressMessages(
  ...
  select(-.data$tidal, -.data$indextypecode, -.data$zonation_old) %>%
  #mutate()
...
suppressMessages(
  ...
  filter(.data$indextypecode == "ZTWA") %>%
  select(-.data$tidal, -.data$indextypecode, -.data$zonation) %>%
  #mutate()
...
taxonlist %>%
  select(.data$taxonname, .data$taxoncode, starts_with("WF_")) %>%
  kable()
```
...
columns_relevant <- taxonlist %>%
  select(
    -.data$taxonname, -.data$taxoncode, -starts_with("WF_"),
