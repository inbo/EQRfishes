...
if (!is.na(measure_rules$SalTru_to_SalFar) & !measure_rules$SalTru_to_SalFar) {
  cat("SAL.FAR. and SAL.TRU. are treated as separate species/variants to calculate this metric; SAL.TRT. is treated as the same species as SAL.FAR.\n\n")
} else {
  if (sal_present) {
    cat("SAL.FAR., SAL.TRU. and SAL.TRT. are treated as one species (and values of SAL.FAR. are used).\n\n")
  }
}
```
